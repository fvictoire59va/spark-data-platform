...
from common.writers import ParquetWriter, JSONWriter, WriteMode
...
class SalesQualityChecksJob(SparkJob):
    ...
    def _check_email_validity(
        ...
        if invalid_emails > 0:
            invalid_examples = [
                row[email_column] for row in 
                df.filter(
                    F.col(email_column).isNotNull() & 
                    ~F.col(email_column).rlike(email_pattern)
                ).select(email_column).limit(5).collect()
            ]
    ...
    def _check_value_distribution(
        ...
        # Valeurs valides
        valid_count = sum(
            count for value, count in distribution_dict.items()
            if value in expected_values
        )
        
        # Valeurs inattendues
        unexpected_values = {
            value: count for value, count in distribution_dict.items()
    ...
    def _check_temporal_distribution(
        ...
        if len(distribution) >= 2:
            months = sorted(distribution.keys())
...
def main():
    ...
    parser.add_argument("--fail-on-error", action="store_true", help="Échouer si qualité insuffisante")
    ...
    try:
        ...
        print(f"Checks passés: {result.metrics.get('passed_checks', 0)}/{result.metrics.get('total_checks', 0)}")
        print(f"{'='*60}\n")
        ...
        if args.fail_on_error:
            ...
            if score < args.min_score:
                raise SystemExit(f"Score de qualité insuffisant: {score:.2%} < {args.min_score:.2%}")
