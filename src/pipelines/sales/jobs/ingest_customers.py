...
class IngestCustomersJob(SparkJob):
    ...
    def transform(self, df: DataFrame, context: JobContext) -> DataFrame:
        ...
        df = df.filter(F.col("customer_id").isNotNull() & (F.trim(F.col("customer_id")) != ""))
    ...
    def _clean_text_columns(self, df: DataFrame) -> DataFrame:
        ...
        for col_name in text_columns:
            if col_name in df.columns:
                df = df.withColumn(
                    col_name,
                    F.when(F.col(col_name).isNotNull(), F.trim(F.col(col_name))).otherwise(None),
                )
        ...
        for col_name in boolean_columns:
            if col_name in df.columns:
                df = df.withColumn(
                    col_name,
                    F.when(
                        F.lower(F.col(col_name)).isin(["true", "1", "yes", "oui", "y"]), F.lit(True)
                    )
                    .when(
    ...
    def _standardize_values(self, df: DataFrame) -> DataFrame:
        ...
        if "gender" in df.columns:
            df = df.withColumn(
                "gender",
                F.when(F.upper(F.col("gender")).isin(["M", "MALE", "HOMME", "H"]), F.lit("M"))
                .when(F.upper(F.col("gender")).isin(["F", "FEMALE", "FEMME"]), F.lit("F"))
        ...
        if "customer_segment" in df.columns:
            df = df.withColumn(
                "customer_segment",
                F.when(F.col("customer_segment").isNull(), F.lit("STANDARD")).otherwise(
                    F.upper(F.col("customer_segment"))
                ),
            )
        ...
        if "preferred_language" in df.columns:
            df = df.withColumn(
                "preferred_language",
                F.when(F.col("preferred_language").isNull(), F.lit("en")).otherwise(
                    F.lower(F.col("preferred_language"))
                ),
            )
    ...
    def validate(self, df: DataFrame, context: JobContext) -> DataFrame:
        ...
        rules = [
            ...
            QualityRule(
                ...
                column="country",
                rule_type="custom",
                parameters={"expression": "country IS NULL OR length(country) BETWEEN 2 AND 3"},
        ...
        for result in validation_result.results:
            if not result.passed:
                log_method = (
                    logger.error
                    if result.severity == QualitySeverity.CRITICAL
                    else logger.warning
                    if result.severity == QualitySeverity.WARNING
                    else logger.info
                )
                log_method(
