...
class GreatExpectationsRunner:
    ...
    def _expect_column_values_to_be_in_set(
        ...
        out_of_set = (
            df.filter(~F.col(column).isin(value_set)).select(column).distinct().limit(10).collect()
        )
    ...
    def _expect_column_values_to_match_regex(
        ...
        non_matches = (
            df.filter(~F.col(column).rlike(regex)).select(column).distinct().limit(5).collect()
        )
    ...
    def list_available_suites(self) -> list[str]:
        ...
        return [f.stem for f in self.expectations_path.glob("*.json")]
